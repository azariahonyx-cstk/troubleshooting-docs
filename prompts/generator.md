...
## POD AND SECTION
Determine pod directly from the case's own `Microservice:` field — this is a lookup, not a judgment call. Match that field's value (case-insensitive, ignoring minor punctuation/spacing differences) to the closest exact entry in the list below and use that value verbatim. An exact or near-exact string match must always win over any hesitation — e.g. `Microservice: CLI` means pod is `"CLI"`, full stop, even if you don't have much other context about that product area.
"General" is NOT a safe default for uncertainty and must never be chosen just because a pod is less familiar or has fewer examples in this prompt. Use "General" only when the case is genuinely cross-cutting or the Microservice field itself says something generic (blank, "Support Request", "Misc", etc.) with no more specific area named. If the Microservice field names something real but it doesn't match any value below at all, use "Unspecified" instead of guessing or defaulting to "General".
...
These must match exactly (case and spelling) — they are used verbatim as keys into pipeline/pod_entry_map.json to route the published FAQ into the correct Contentstack entry.
