...
2. EXTRACTED FACTS (if provided) — the generator's own extraction of problem/root_cause/resolution_steps/fix_confirmed, each explicitly marked "Not mentioned in notes." where the case didn't state it. Treat this as the ground truth for what's actually supported — it's more reliable than re-deriving facts from the current draft's prose.
3. CURRENT DRAFT — the article body_markdown as currently written.
4. VERIFIER FINDINGS — hallucinations (claim + problem), missing_info, sensitive_data (found + replace_with), and revision_notes (specific fixes).
...
- Every hallucinated claim listed must be removed or reduced to only what the raw case (and extracted facts, if provided) actually supports. Do not invent a replacement explanation, mechanism, number, or version to fill the gap.
...
- Resolution step count must still exactly match the extracted resolution_steps (if provided) or what the raw case describes — do not add or merge steps while revising.
...
- Preserve the article's format exactly: same H1 title, problem statement (no "Users/A user/The customer..." openers), "## Root cause", "## Resolution" (numbered, direct instructions to the reader — never "Informed/Advised/Validated/Shared/Explained/Instructed"), "## Verification" (starts "After completing these steps, ..."; only include specific verification/escalation mechanics — payloads, headers, replay steps, data to capture — if the raw case explicitly states them, otherwise use the generic default: "After completing these steps, confirm the issue no longer occurs. If it persists, escalate with relevant details (logs, version, screenshots, configuration) to Contentstack support.").
- If a hallucination finding is about the Verification section specifically, do not just remove the flagged sentence and leave a gap — replace it with the generic default above. Never invent a *different* specific procedure to replace one that was just flagged; that repeats the same mistake in a new form.
