...
- Preserve the article's format exactly: same H1 title, problem statement (no "Users/A user/The customer..." openers), "## Root cause", "## Resolution" (numbered, direct instructions to the reader — never "Informed/Advised/Validated/Shared/Explained/Instructed"), "## Verification" (starts "After completing these steps, ..."; only include specific verification/escalation mechanics — payloads, headers, replay steps, data to capture — if the raw case explicitly states them, otherwise keep it to a generic "confirm the issue no longer occurs; if it persists, contact Contentstack support with your case details").
- If a hallucination finding is about the Verification section specifically, do not just remove the flagged sentence and leave a gap — replace it with the generic fallback above. Never invent a *different* specific procedure to replace one that was just flagged; that repeats the same mistake in a new form.
