...
meta_description: "Mission Control deployment alerts can stop arriving in a team's notification channel when that channel is renamed on the connected messaging platform, breaking the webhook link. Learn how to reconnect the integration."
status: "draft"
...
Mission Control deployment alerts may stop reaching the assigned team's notification channel when that channel is renamed on the connected messaging platform, breaking the existing webhook link.

## Root cause

The notification channel had been renamed on the customer's messaging platform, which broke the existing webhook link connecting it to Mission Control.
