...
<!-- case:00060524 status:draft synced:false bucket:"Content Editing & UI Workflows" -->
### CLI Module Import Fails Without a Shared Backup Directory

Running Contentstack CLI module-by-module imports independently may cause cascading failures, including missing extension references, entry publish errors, and global field creation failures.

**Root Cause**

Each CLI import module run without a shared `--backup-dir` flag creates its own isolated working copy with no memory of IDs (such as extensions) created by earlier modules, breaking the cross-module ID handoff that later modules depend on. Additionally, `--backup-dir` is not a pre-created source path — it is auto-generated by the CLI during the first module import to store mapping files for dependent modules; pointing it at a non-existent path causes the CLI to find nothing there and return a false success with no data actually imported.

**Resolution**

1.  Run the first module import (Locales) without the `--backup-dir` flag, allowing the CLI to auto-generate the backup directory (typically named `_backup_<timestamp>`).

2.  Pass `--backup-dir`, pointing to the auto-generated folder, on all subsequent module imports in dependency order: Locales, Environments, Assets, Taxonomies, Extensions, Marketplace Apps, Webhooks, Global Fields, Content Types, Personalize, Workflows, Entries, Labels, Custom Roles, Studio.

3.  Verify each module completes successfully before proceeding to the next.

After re-running the import sequence with a shared backup directory, confirm entries publish without errors and that extension and global field references resolve correctly. If no publish or reference errors remain, the issue is resolved. Escalate with the module sequence used and specific error counts if failures persist.

<!-- end:00060524 -->

<!-- case:00060532 status:draft synced:false bucket:"Content Editing & UI Workflows" -->
### Task Assignment Emails Misattribute the Assigning User

Workflow task assignment notification emails may name a colleague as the person who assigned a task, even when that colleague is not configured as the assignee in the workflow stage.

**Root Cause**

The task notification email and the "Assigned On" column reflect the user who triggered the workflow stage transition, not necessarily the user explicitly configured as the assignee in the stage's Assign To list. When a role-based approver list includes users who did not intend to be approvers, tasks and notification emails are generated for them even though they never manually assigned the task.

**Resolution**

1.  Navigate to Settings > Workflows, open the workflow containing the affected stage, and review its "Assign To" list of users and roles.

2.  Remove any colleagues or roles from the "Assign To" list who should not receive task assignments or notification emails for that stage.

3.  If tasks are instead generated by a publish rule rather than a workflow stage, apply the same approver-list narrowing under Settings > Publish Rules.

4.  For tasks that already exist in a user's queue, advance the related entry to its next workflow stage or have an admin approve or reject it; there is no bulk-delete option for existing tasks.

After narrowing the Assign To list and clearing existing tasks by advancing their entries, confirm no new unintended task assignment emails are generated. If notifications stop appearing for the removed approvers, the issue is resolved. Escalate with the workflow or publish rule name if unintended assignments continue.
...
<!-- case:00059744 status:draft synced:false bucket:"API Delivery, GraphQL & Assets" -->
### GraphQL Missing _metadata.uid for Nested Global Fields

Querying a Global Field nested inside a Modular Block through the GraphQL API may return a response missing the `_metadata.uid` field, even though the field is present and correctly populated.

**Root Cause**

This was a confirmed platform bug in how the GraphQL API resolved and exposed `_metadata.uid` for Global Fields nested inside Modular Blocks. Contentstack engineering deployed a fix to expose this field correctly.

**Resolution**

1.  Confirm you are hitting this issue by checking whether GraphQL responses for Global Fields nested inside Modular Blocks omit the `_metadata.uid` field while other fields resolve correctly.

2.  Re-run the affected GraphQL query. Contentstack engineering has deployed a fix to expose `_metadata.uid` for Global Fields nested inside Modular Blocks.

3.  Contact Contentstack Support with the content type UID and a sample GraphQL query if `_metadata.uid` is still missing after retrying.

After retrying the query, confirm `_metadata.uid` is now present in the GraphQL response for the affected Global Field. If the field appears correctly, the issue is resolved. Escalate with your content type UID and query if it persists.
...
<!-- case:00060807 status:draft synced:false bucket:"Custom Extensions, Live Preview & Analytics" -->
### CDN Bandwidth Spike From a Single Video Asset

A single large video asset embedded on a high-traffic page may drive a disproportionate CDN bandwidth spike, far exceeding what its file size alone would suggest.

**Root Cause**

Native mobile video players (such as iOS's) can aggressively prefetch and re-request video segments during playback and seeking. This prefetch and seek behavior can multiply the effective transfer volume of a single video asset far beyond its base file size, producing a large bandwidth spike from what appears to be one modest-sized file.

**Resolution**

1.  Use Product Analytics' Top URLs report to identify which asset URLs are driving the bandwidth spike.

2.  Confirm whether the top-consuming URL is a video asset and review its playback and seek pattern in traffic logs.

3.  Reduce the amplification by switching to adaptive streaming, compressing the video, or serving it from a dedicated video CDN or streaming service rather than as a direct progressive-download asset.

After implementing adaptive streaming, compression, or a dedicated video delivery service, monitor CDN bandwidth over the following days to confirm the spike has subsided. If bandwidth returns to expected levels, the issue is resolved. Escalate with the Top URLs report and affected asset UID if consumption remains elevated.
...
<!-- case:00060935 status:draft synced:false bucket:"Assets & Metadata Management" -->
### False Dimension Validation Error Blocks Entry Publish

Publishing an entry may fail with a dimension validation error even when the associated file field has no image dimension restrictions configured.

**Root Cause**

Root cause was not identified in the source case data. Investigation found no default or undocumented image dimension restriction responsible for the error; the validation was generated client-side by the CMS UI before the publish request reached the CMA. The resolution below reflects the steps that resolved the reported symptom, which pointed to an issue with the original asset upload rather than a dimension configuration.

**Resolution**

1.  Create a new asset entry to replace the one that returned the dimension validation error.

2.  Attach the newly created asset to the affected file field in place of the original.

3.  Publish the entry again.

After replacing the asset and republishing, confirm the entry publishes without the dimension validation error. If publishing succeeds, the issue is resolved. Escalate with the affected entry UID and a screenshot of the error if it persists after recreating the asset.
